...
extra_state_dict = torch.load("pretrained/yolox_x.pth")
extra_state_dict_1 = torch.load("pretrained/yolox_x.pth")
extra_state_dict_2 = torch.load("pretrained/yolox_x.pth")
extra_state_dict_3 = torch.load("pretrained/yolox_x.pth")
for k in list(extra_state_dict["model"].keys()):
    if "head" in k:
        state_dict["model"][k.replace("head", "extra_head")] = extra_state_dict["model"][k]
for k in list(extra_state_dict_1["model"].keys()):
    if "head" in k:
        state_dict["model"][k.replace("head", "extra_head_1")] = extra_state_dict_1["model"][k]
for k in list(extra_state_dict_2["model"].keys()):
    if "head" in k:
        state_dict["model"][k.replace("head", "extra_head_2")] = extra_state_dict_2["model"][k]
for k in list(extra_state_dict_3["model"].keys()):
    if "head" in k:
        state_dict["model"][k.replace("head", "extra_head_3")] = extra_state_dict_3["model"][k]
torch.save(state_dict, "pretrained/yolox_s_with_4_head_x.pth.tar")
print(state_dict["model"].keys())