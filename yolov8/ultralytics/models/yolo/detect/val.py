...
class DetectionValidatorCustom(DetectionValidator):
    """Validate YOLOv8 inference on a single dataset."""
    @smart_inference_mode()
    def __call__(self, trainer=None, model=None):
        """Supports validation of a pre-trained model if passed or a model being trained if trainer is passed (trainer
        gets priority).
        """
        print("Run Custom Validation")
        ...
        for batch_i, batch in enumerate(bar):
            ...
            # Inference
            with dt[1]:
                preds = model(batch["img"], augment=augment, is_training = False)
