...
def main(exp, args):
    ...
    with open("datasets/mix_det/annotations/train.json", "r") as f:
        train_annos = json.load(f)

    feature_vectors = []
    for image in train_annos["images"]:
        url = "datasets/mix_det/" + image["file_name"]
        print(url)
        img = cv2.imread(url)
        img, ratio = preproc(img, exp.test_size, (0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
        img = torch.from_numpy(img).unsqueeze(0).float().to(args.device)
        if args.fp16:
            img = img.half()  # to FP16
        with torch.no_grad():
            fpns = model.backbone(img)
            squeeze_fpns = list(fpns)
            for i in range(len(squeeze_fpns)):
                bs, c, _, _ = squeeze_fpns[i].shape
                squeeze_fpns[i] = nn.AdaptiveAvgPool2d(1)(squeeze_fpns[i]).view(bs, c)
            feature_vector = torch.cat(squeeze_fpns, dim=1)
