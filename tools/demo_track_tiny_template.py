...
def remove_array(arrays, array_to_remove):
    for i, arr in enumerate(arrays):
        if np.array_equal(arr, array_to_remove):
            arrays.pop(i)
            return
    raise ValueError("Array not found in the list.")

def nms(boxes, iou_threshold=0.5):
    bbox_list_thresholded = boxes	# List to contain the boxes after filtering by confidence
    ...
    while len(bbox_list_thresholded) > 0:
        ...
        for box in bbox_list_thresholded:
            iou = get_iou(current_box[:4], box[:4])		# Calculate the IOU of the two boxes
            if iou > iou_threshold:					# Check if the iou is greater than the threshold defined
                remove_array(bbox_list_thresholded, box)	# If there is significant overlap, then remove the box
    return bbox_list_new


def imageflow_demo_tiny_template(predictor,smaller_predictor, vis_folder, current_time, args):
    ...
    while True:
        ...
        if ret_val:
            ...
            if frame_id % 10 == 0 or countdown_yolo > 0:
                ...
                outputs, img_info = smaller_predictor.inference(frame, timer)
                output = outputs[0]
            ...
            if outputs[0] is not None:
                ...
                save_online_targets_img = {}
                for t in online_targets:
                    tl = (max(0, min(int(t.tlbr[0]), img_info["width"])), max(0, min(int(t.tlbr[1]), img_info["height"])))
                    br = (max(0, min(int(t.tlbr[2]), img_info["width"])), max(0, min(int(t.tlbr[3]), img_info["height"])))
                    detect_img = img_info["raw_img"][tl[1]:br[1], tl[0]:br[0]]
                ...
                for t in online_targets:
                    ...
                    if tlwh[2] * tlwh[3] > args.min_box_area and not vertical:
                        ...
                        # cv2.rectangle(test_img, (int(tlwh[0]), int(tlwh[1])), (int(tlwh[0] + tlwh[2]), int(tlwh[1] + tlwh[3])), (255, 0, 255), 2)
                timer.toc()
                online_im = plot_tracking(
            ...
            break
        # cv2.imshow('image', cv2.resize(test_img, (test_img.shape[1] // 2, test_img.shape[0] // 2)))
        # cv2.waitKey(0)
        frame_id += 1
...
def main(exp,smaller_exp, args):
    ...
    if args.demo == "image":
        ...
        imageflow_demo_tiny_template(predictor, smaller_predictor, vis_folder, current_time, args)
