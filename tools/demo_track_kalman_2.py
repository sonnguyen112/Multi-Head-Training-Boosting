...
def imageflow_demo_kalman_2(predictor, vis_folder, current_time, args):
    cap = cv2.VideoCapture(args.path if args.demo == "video" else args.camid)
    width = cap.get(cv2.CAP_PROP_FRAME_WIDTH)  # float
    height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)  # float
    fps = cap.get(cv2.CAP_PROP_FPS)
        ...
        if ret_val:
            ...
            if outputs[0] is not None:
                ...
                if is_use_yolo:
                    save_online_targets_img = []
                    for t in online_targets:
                        detect_img = img_info["raw_img"][max(0, int(t.tlbr[1])):min(
                            img_info["height"], int(t.tlbr[3])), max(0, int(t.tlbr[0])):min(img_info["width"], int(t.tlbr[2]))]
...
def main(exp, args):
    ...
    if args.demo == "image":
        ...
        imageflow_demo_kalman_2(predictor, vis_folder, current_time, args)
