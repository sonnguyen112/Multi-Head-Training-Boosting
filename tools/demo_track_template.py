...
def thread_util(save_online_targets_img, i, stracks, img_info, outputs_dict, r, sim_list, threshold_to_open_yolo):
    ...
    crop_h, crop_w = save_online_targets_img[stracks[i].track_id].shape[:2]
    ...
    result = cv2.matchTemplate(crop_extend, save_online_targets_img[stracks[i].track_id], cv2.TM_CCOEFF_NORMED)
    ...
    h, w, _ = save_online_targets_img[stracks[i].track_id].shape
    ...
    if sim_score > threshold_to_open_yolo:
        save_online_targets_img[stracks[i].track_id] = output_img
...
def predict_new_detection_template(stracks, img, predictor, timer, save_online_targets_img):
    ...
    for i, output in enumerate(stracks):
        if output.track_id not in save_online_targets_img:
            continue
        thread = threading.Thread(target=thread_util, args=(
            save_online_targets_img, i, stracks, img_info, outputs_dict, r, sim_list, threshold_to_open_yolo))
    ...
    for strack in stracks:
        if strack.track_id not in outputs_dict:
            continue
...
def imageflow_demo_template(predictor, vis_folder, current_time, args):
...
def main(exp, args):
    ...
    if args.demo == "image":
        ...
        imageflow_demo_template(predictor, vis_folder, current_time, args)
