...
def predict_new_detection(stracks, img, predictor, timer):
    ...
    for i,output in enumerate(outputs):
        ...
        # print("Sim", calculate_similarity_color_histogram(kalman_img, detect_img))
        avg_similarity += calculate_similarity_color_histogram(kalman_img, detect_img)
        cv2.waitKey(0)
    ...
    timer.toc()
    return [outputs], img_info, iou_matrix, avg_similarity
    ...
    while True:
        ...
        if ret_val:
            ...
            if outputs[0] is not None:
                ...
                # print('this is output', outputs[0])
                # exit(0)
                online_im = plot_tracking(online_targets,
                                          outputs[0], img_info['raw_img'], online_tlwhs, online_ids, frame_id=frame_id + 1, fps=1. / timer.average_time,
