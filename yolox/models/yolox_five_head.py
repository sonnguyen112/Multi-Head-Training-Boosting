...
class YOLOXFiveHead(nn.Module):
    ...
    def __init__(self, backbone=None, head=None, extra_head=None, extra_head_1=None, extra_head_2=None, extra_head_3=None):
        super().__init__()
        if backbone is None:
            backbone = YOLOPAFPN()
        if head is None:
        ...
        if extra_head_2 is None:
            extra_head_2 = YOLOXHead(80)
        if extra_head_3 is None:
            extra_head_3 = YOLOXHead(80)

        self.backbone = backbone
        self.head = head
        self.extra_head = extra_head
        self.extra_head_1 = extra_head_1
        self.extra_head_2 = extra_head_2
        self.extra_head_3 = extra_head_3
        ...
        self.up_channels_2 = nn.Sequential(
            nn.Conv2d(int(256 * self.head.width), int(256 * self.extra_head_2.width), kernel_size=1),
            nn.Conv2d(int(512 * self.head.width), int(512 * self.extra_head_2.width), kernel_size=1),
            nn.Conv2d(int(1024 * self.head.width), int(1024 * self.extra_head_2.width), kernel_size=1),
        )

        self.up_channels_3 = nn.Sequential(
            nn.Conv2d(int(256 * self.head.width), int(256 * self.extra_head_3.width), kernel_size=1),
            nn.Conv2d(int(512 * self.head.width), int(512 * self.extra_head_3.width), kernel_size=1),
            nn.Conv2d(int(1024 * self.head.width), int(1024 * self.extra_head_3.width), kernel_size=1),
        )
    ...
    def forward(self, x, targets=None):
        ...
        if self.training:
            ...
            extra_fpn_outs_2 = list(fpn_outs)
            for i in range(len(extra_fpn_outs_2)):
                extra_fpn_outs_2[i] = self.up_channels_2[i](extra_fpn_outs_2[i])
            extra_fpn_outs_2 = tuple(extra_fpn_outs_2)
            extra_loss_2, extra_iou_loss_2, extra_conf_loss_2, extra_cls_loss_2, extra_l1_loss_2, extra_num_fg_2 = self.extra_head_2(
                extra_fpn_outs_2, targets, x
            )

            extra_fpn_outs_3 = list(fpn_outs)
            for i in range(len(extra_fpn_outs_3)):
                extra_fpn_outs_3[i] = self.up_channels_3[i](extra_fpn_outs_3[i])
            extra_fpn_outs_3 = tuple(extra_fpn_outs_3)
            extra_loss_3, extra_iou_loss_3, extra_conf_loss_3, extra_cls_loss_3, extra_l1_loss_3, extra_num_fg_3 = self.extra_head_3(
                extra_fpn_outs_3, targets, x
            )

            outputs = {
                "total_loss": loss + extra_loss + extra_loss_1 + extra_loss_2 + extra_loss_3,
                "loss":loss,
