        ...
        if self.training:
            ...
            for extra_index, extra_head in enumerate(self.extra_heads):
                ...
                for i in range(len(extra_fpn_outs)):
                    ...
                    normalize_student = (extra_fpn_outs[i] - extra_fpn_outs[i].min()) / (extra_fpn_outs[i].max() - extra_fpn_outs[i].min())
                    normalize_teacher = (t_fpn_outs[i] - t_fpn_outs[i].min()) / (t_fpn_outs[i].max() - t_fpn_outs[i].min())
                    total_extra_loss += torch.dist(normalize_student, normalize_teacher, p=2) * 0.05
