...
def count_num_objects(output_results, track_thresh):
    ...
    return remain_inds.sum()

class MOTEvaluatorSecondary:
    """
    ...
    def evaluate(
        ...
        model = model.eval()
        secondary_model = secondary_model.eval()
        if half:
            model = model.half()
            secondary_model = secondary_model.half()
        ids = []
        data_list = []
        results = []
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                ...
                if is_use_secondary_model:
                    outputs = secondary_model(imgs)
                else:
                    outputs = model(imgs)
                ...
                num_object = count_num_objects(outputs[0], self.args.track_thresh)
                if num_object < 10:
                    is_use_secondary_model = True
                else:
                    is_use_secondary_model = False
